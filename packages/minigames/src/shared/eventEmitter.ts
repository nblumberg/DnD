interface DataChangeHandler<OBJECT_OR_PROPERTY_VALUE> {
  (data: OBJECT_OR_PROPERTY_VALUE): void;
}

export interface AddListener<OBJECT> {
  (handler: DataChangeHandler<OBJECT>): OBJECT;
}

export interface AddPropertyListener<OBJECT> {
  <PROPERTY extends keyof OBJECT>(
    property: PROPERTY,
    handler: DataChangeHandler<OBJECT[PROPERTY]>
  ): OBJECT[PROPERTY];
}

export interface RemoveListener<OBJECT> {
  <PROPERTY extends keyof OBJECT>(
    handler?: DataChangeHandler<OBJECT> | DataChangeHandler<OBJECT[PROPERTY]>,
    property?: PROPERTY
  ): void;
}
...
export function createEventEmitter<T, P extends keyof T>(
  data: T
): {
  ...
  const dataChangeListeners = new Map<
    keyof T | typeof allData,
    Set<DataChangeHandler<T> | DataChangeHandler<T[P]>>
  >();

  function addHandler(
    key: keyof T | typeof allData,
    handler: DataChangeHandler<T> | DataChangeHandler<T[P]>
  ): void {
  ...
  return {
    ...
    addPropertyListener(property, handler) {
      addHandler(property, handler as unknown as DataChangeHandler<T[P]>);
    ...
    removeListener(handler, property) {
      ...
      if (handler) {
        dataChangeListeners
          .get(key)!
          .delete(
            handler as unknown as DataChangeHandler<T> | DataChangeHandler<T[P]>
          );
      } else {
      ...
      for (const property of dataChangeListeners.keys()) {
        if (property === allData) {
          for (const listener of dataChangeListeners.get(property)!.values()) {
            (listener as DataChangeHandler<T>)({ ...data });
          }
        } else {
