            ...
            function Listener() {
                ...
                this.createCard = function(creature, i, total) {
                    creature.createCard({
                        ...
                        className: "gridItem",
                        cardSize: jQuery(w).height() / total, // Math.floor(Math.sqrt((screen.availWidth * screen.availHeight) / total) - 100),
                        showPcHp: true
                    });
                };
                ...
                this.changeTurn = function(data) {
                    var i, actor;
                    out.console.debug("Received \"changeTurn\" message from " + this.current + " to " + data.current);
                ...
                this.pause = function(data) {
                    var i, actor;
                    out.console.debug("Received \"pause\" message");
                    actor = this.findActor(this.current);
                    actor.card.pause();
                };
                ...
                this.restart = function(data) {
                    var i, actor;
                    out.console.debug("Received \"restart\" message");
                    actor = this.findActor(this.current);
                    actor.card.restart();
                };
                ...
                this.highlightImage = function(data) {
                    ...
                    out.console.debug("Received \"highlightImage\" message { x: " + (100 * data.position.x) + "%, y: " + (100 * data.position.y) + "% }");
                ...
                jQuery(w.document).ready((function() {
                    var i;
                    out.console.debug("DOM ready");
                    if (w.opener) {
                        w.opener.postMessage("ready", "*");
                    }
                    ...
                    w.setInterval(this.handleMessage.bind(this), 1000);
                }).bind(this));
            }
