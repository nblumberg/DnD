/* global DnD:true, Serializable, EventDispatcher, safeConsole */
...
(function() {
    ...
    DnD.define(
        "History",
        [ "window", "Serializable", "jQuery" ],
        function(w, Serializable, jQuery) {
            ...
            function History(params) {
                History.histories.push(this);
                this.id = History.histories.length - 1;
            ...
            History.get = function($child) {
                return History.histories[ parseInt($child.closest(".history").attr("data-history-id"), 10) ];
            };
            ...
            if (!History._eventHandlersInitialized) {
                jQuery(w.document).on("click", "button.expandCollapseAll", function() {
                    var $button, history, $ul, expand;
                    $button = jQuery(this);
                    history = History.get($button);
                    if (!history) {
                        return;
                    }
                ...
                }).on("click", "li.round", function() {
                    jQuery(this).children("ul").toggle();
                }).on("click", "li.entry", function(event) {
                    var $entry, history;
                    event.stopPropagation();
                    $entry = jQuery(this);
                    history = History.get($entry);
                    if (!history) {
                        return;
                    }
            ...
            History.prototype._createHtml = function() {
                ...
                this.$html = $html;
                this.$html.attr("data-history-id", this.id);
                return this.$html;
            };
    ...
    DnD.define(
        "History.Editor",
        [ "History", "jQuery", "Serializable" ],
        function(History, jQuery, Serializable) {
            // HISTORY.EDITOR
            // CONSTRUCTOR
    ...
    DnD.define(
        "History.Entry",
        [ "History", "out", "jQuery", "Serializable", "EventDispatcher" ],
        function(History, out, jQuery, Serializable, EventDispatcher) {
            // HISTORY.ENTRY
            // CONSTRUCTOR
            ...
            History.Entry.prototype._getSubjectName = function(warn) {
                if (typeof(this.subject) === "number") {
                    ...
                    this.subject = DnD.Creature.actors[ this.subject ] ? DnD.Creature.actors[ this.subject ] : this.subject;
                }
            ...
            History.Entry.prototype._edit = function($entry) { // $entry, history
            ...
            History.Entry.prototype._save = function(instance, history, event) {
                ...
                this._render(instance, history._includeSubject); // TODO: history is undefined?
