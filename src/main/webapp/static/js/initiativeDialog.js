/* global define */
/* exported DnD.Dialog.Initiative */
(function() {
    "use strict";

    define({
        name: "Dialog.Initiative",
        dependencyNames: [ "Dialog", "Roll", "jQuery", "console" ],
        factory: function(Dialog, Roll, jQuery, console) {
            ...
            InitiativeDialog.prototype = new Dialog("initiativeDialog", "/html/partials/initiativeDialog.html");
            ...
            InitiativeDialog.prototype._populate = function() {
                ...
                for (i = 0; i < this.order.length; i++) {
                    ...
                    jQuery("<div/>").addClass("name").html(actor.name).appendTo($a);
                    roll = this.rolls && this.rolls.length === this.order.length ? this.rolls[ i ] : this.order.length - i;
                    jQuery("<input/>").attr("type", "number").attr("min", "1").attr("step", 1).addClass("order").attr("name", actor.id).val(roll).attr("placeholder", roll).appendTo($a);
                }
            };
