    ...
    DnD.define(
        ...
        function(
            ...
            function Initiative(params) {
                this.__log = out.logFn.bind(this, "Initiative");
            ...
            Initiative.prototype._init = function(params) {
                ...
                if (!params) {
                    out.logFn("Storage", "read", [ "initiative" ]);
                    this.__storage.read("initiative", function(data) {
                        if (data) {
                            out.console.info("Loaded from storage");
                        }
                        else {
                            data = {};
                        }
                ...
                params = jQuery.extend(
                    ...
                    params,
                    { creatures: jQuery.extend({}, party, monsters) }
                );
                ...
                if (params.historyEntries) {
                    HistoryEntry.init(params.historyEntries); // NOTE: must come before this.actors is initialized because Creature.history references it
                }
            ...
            Initiative.prototype._createHistory = function() {
                ...
                if (!this.freeFormHistory) {
                    this.freeFormHistory = new HistoryEditor({
            ...
            Initiative.prototype._addActor = function(creature, actors, currentState) {
                ...
                if (!creature) {
                    out.console.warn("Skipping adding undefined actor");
                    return;
                }
                else if (!creature.name) {
                    out.console.warn("Skipping adding invalid actor (missing name)");
                    return;
                }
                else if (!creature.image) {
                    out.console.warn("Skipping adding invalid actor (missing image)");
                    return;
                }
                if (!this.actors) {
                    this.actors = [];
                ...
                actor.addEventListener("change", function(event) {
                    ...
                    actor = event.target;
                    out.console.debug(actor.name + "'s " + event.property + " changed from " + event.oldValue + " to " + event.newValue);
            ...
            Initiative.prototype.POST_MESSAGE = {
                LISTEN: w.addEventListener ? "addEventListener" : "attachEvent",
                STOP: w.removeEventListener ? "removeEventListener" : "detachEvent",
                TYPE: w.addEventListener ? "message" : "onmessage"
            };
            ...
            Initiative.prototype._messageDisplay = function(msg, createDisplay) {
                ...
                if (createDisplay && !(this.display && !this.display.closed)) {
                    ...
                    this.display = w.open("initiative.html", "Initiative", "location=0,status=0,toolbar=0", false);
                    w[ this.POST_MESSAGE.LISTEN ](this.POST_MESSAGE.TYPE, this._displayLoadHandler.bind(this), false);
                    this.display.focus();
                }
                if (this.display) { // TODO: label postMessage events so listener only handles them once in case single dispatch causes multiple listener receptions
                    ...
                    this.display.postMessage(json, "*"); // TODO: security
                }
                return json;
            };
            ...
            Initiative.prototype._displayLoadHandler = function(event) {
                ...
                out.console.info("Display loaded");
                ...
                intervalId = w.setInterval(function() {
                    if (this.display.closed) {
                        w.clearInterval(this.$displayButton.data("intervalId"));
            ...
            Initiative.prototype._addHistory = function(actor, message, method) {
                ...
                entry = new HistoryEntry({ round: this.round, subject: actor, message: message });
            ...
            Initiative.prototype._exit = function(actor) { // actor, event
                ...
                if (w.confirm("Remove " + actor.name + " from play?")) {
                    index = this.order.indexOf(actor.id);
                    this.order.splice(index, 1);
            ...
            Initiative.prototype.raw = function() {
                ...
                raw = {
                    ...
                    historyEntries: this.rawObj(HistoryEntry.entries),
                    round: this.round,
