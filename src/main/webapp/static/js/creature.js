...
Actor.Card = function(params) {
	...
	this.subPanel = {};
	this.cardSize = params.cardSize || Actor.Card.CARD_SIZE;
	this.$parent = params.$parent ? jQuery(params.$parent) : jQuery("body");
	this.$panel = jQuery("<div/>").attr("id", this.actor.name.replace(/\s/g, "_") + "_panel").addClass("creaturePanel centered bordered " + params.className).appendTo(this.$parent);
	...
	this.subPanel.portrait.src = this.actor.image;
	this.subPanel.$images.append(this.subPanel.portrait);
	this.showPcHp = params.showPcHp;
	this._renderName();
	
    this.subPanel.$effects = jQuery("<div/>").addClass("effects").appendTo(this.subPanel.$images);
    this.updateConditions();
    
    this.damageIndicator = new Actor.Card.DamageIndicator({ actor: this.actor, card: this, $parent: this.subPanel.$effects });
};

Actor.Card.prototype._renderName = function() {
	if (this.subPanel.$name) {
		this.subPanel.$name.remove();
	}
	this.subPanel.$name = jQuery("<div/>").addClass("name f2").html(this.actor.name).appendTo(this.$panel);
	if (this.showPcHp && this.actor.isPC) {
		this.subPanel.$hp = jQuery("<span/>").addClass("hp").html(this.actor.hp.current + (this.actor.hp.temp ? " (" + (this.actor.hp.temp + this.actor.hp.current) + ")" : "") + " / " + this.actor.hp.total);
...
Actor.Card.CARD_SIZE = 240;

/**
 * Display an Actor.Card as being the current turn (or not)
 */
Actor.Card.prototype.makeCurrent = function(isCurrent) {
	if (!this.$panel) {
		return;
	}
...
/**
 * Display an Actor.Card as being bloodied (or not)
 */
Actor.Card.prototype.makeBloodied = function(isBloodied) {
	if (!this.$panel) {
		return;
	}
...
Actor.Card.prototype.refresh = function(isCurrent) {
...
/**
 * Display all of an Actor's Effects on their Actor.Card
 */
Actor.Card.prototype.updateConditions = function() {
...
/**
 * Displays an Effect on an Actor.Card
 * 
 ...
 */
Actor.Card.prototype._renderCondition = function(effect, total) {
 ...
 * @param params Object
 * @param params.actor Actor
 * @param params.card Actor.Card
 * @param params.$parent jQuery(element) The parent element
 * @param params.damage {Damage | Array}
 * @param params.isMiss {Boolean}
 */
Actor.Card.DamageIndicator = function(params) {
 ...
 * @param params.damage {Damage | Array}
 */
Actor.Card.DamageIndicator.prototype.damage = function(damage) {
...
Actor.Card.DamageIndicator.prototype.miss = function() {
    // Clear any existing damage indicator content
    this.$types.html("");
    this.$miss.show();
...
Actor.Card.DamageIndicator.prototype._renderEvent = function(heightPercent, imageSrc, text, color) {
