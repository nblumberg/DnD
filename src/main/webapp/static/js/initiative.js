...
(function() {
    ...
    define({
        name: "Initiative",
        dependencyNames: [ "Storage", "History", "Creature", "Effect", "Roll", "Dialog.Attack", "Dialog.Creature", "Dialog.Export", "Dialog.Heal", "Dialog.Image", "Dialog.Import", "Dialog.Initiative", "Actor", "Attack", "EventDispatcher", "Serializable", "loadParty", "loadMonsters", "jQuery", "console" ],
        factory: function(Storage, History, Creature, Effect, Roll, AttackDialog, CreatureDialog, ExportDialog, HealDialog, ImageDialog, ImportDialog, InitiativeDialog, Actor, Attack, EventDispatcher, Serializable, loadParty, loadMonsters, jQuery, console) {
            ...
            Initiative.prototype._init = function(params) {
                ...
                for (i = 0; i < this.actors.length; i++) {
                    actor = this.actors[ i ];
                    for (j = 0; params.actors[ i ].imposedEffects && j < params.actors[ i ].imposedEffects.length && Effect.effects.hasOwnProperty(params.actors[ i ].imposedEffects[ j ].id); j++) {
                        actor.imposedEffects.push(Effect.effects[ params.actors[ i ].imposedEffects[ j ].id ]);
                    }
                }
            ...
            Initiative.prototype._create = function() {
                ...
                if (!this._created) {
                    ...
                    this.creatureDialog = new CreatureDialog({
                    ...
                    this.initiativeDialog = new InitiativeDialog({
                        actors: this.actors,
                        order: this.order,
                    ...
                    this.attackDialog = new AttackDialog({ callback: function(msg) {
                        this._render(false);
            ...
            Initiative.prototype._countActorsByType = function(type, actors, adding) {
                ...
                for (i = 0; actors && i < actors.length; i++) {
                    ...
                    else if (actor.get("name").indexOf(type) === 0) {
                        potential++;
            ...
            Initiative.prototype._addActor = function(creature, actors, currentState) {
                var count, actor;
                this.__log("_addActor", [ creature.get("name"), actors ? actors.length : 0, currentState ? "currentState" : "undefined" ]);
                //window.alert("stop");
                ...
                count = creature.get("isPC") ? 0 : this._countActorsByType(creature.get("name"), actors, true);
                actor = new Actor(creature, count, currentState, this.__storage);
                this.actors.push(actor);
                jQuery("<option/>").attr("value", actor.get("name")).html(actor.name).data("actor", actor).appendTo(this.$freeFormHistorySubject);
                if (this.order) {
                    this.order.push(actor.get("id"));
                }
                if (History.central) {
                ...
                actor.addEventListener("change", function(event) {
                    ...
                    actor = event.target;
                    console.debug(actor.get("name") + "'s " + event.property + " changed from " + event.oldValue + " to " + event.newValue);
...
//                  this._changeInitiative({ move: move, before: before });
            ...
            Initiative.prototype._addHistory = function(actor, message, method) {
                ...
                entry = new History.Entry({ round: this.round, subject: actor, message: message });
            ...
            Initiative.prototype._rest = function(isExtendedRest) {
                ...
                for (i = 0; i < this.actors.length; i++) {
                    ...
                    for (j = 0; j < actor.attacks.length; j++) {
                        ...
                        if (attack.used && (isExtendedRest || attack.usage.frequency !== Attack.prototype.USAGE_DAILY)) {
                            attack.used = false;
                        }
                    }
            ...
            Initiative.prototype.raw = function() {
                ...
                actors = [];
                for (i = 0; i < this.actors.length; i++) {
                    actors.push({ id: this.actors[ i ].get("id"), name: this.actors[ i ].get("name"), type: this.actors[ i ].get("type") });
                }
                raw = {
                    order: this.order,
                    actors: actors, // this.rawArray(this.actors),
