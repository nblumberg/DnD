/* global define */
/* exported DnD.Dialog.Attack */
(function() {
    "use strict";

    define({
        name: "Dialog.Attack",
        dependencyNames: [ "Dialog", "Attack", "Creature", "jQuery", "console" ],
        factory: function(Dialog, Attack, Creature, jQuery, console) {
            ...
            AttackDialog.prototype.show = function(params) {
                ...
                if (!params || !params.attacker || !params.actors) {
                    console.warn("AttackDialog.show() invoked without sufficient parameters");
                    return;
                }
                ...
                if (needsWeapon || needsImplement) {
                    ...
                    for (i = 0; items && i < items.length; i++) {
                        if (needsWeapon && (isMelee === true || isRanged === true)) {
                            if (!(items[ i ] instanceof Creature.Weapon) || (isMelee && !items[ i ].isMelee) || (isRanged && items[ i ].isMelee === true)) { // TODO: if ((items[ i ].isMelee && !isMelee) || (!items[ i ].isMelee && !isRanged)) {
                                continue;
                            }
                        }
                        else if (needsImplement && !(items[ i ] instanceof Creature.Implement)) {
                            continue;
                        }
