...
(function() {
    ...
    define({
        name: "Listener",
        dependencyNames: [ "Effect", "Actor", "loadParty", "loadMonsters", "jQuery", "console" ],
        factory: function(Effect, Actor, loadParty, loadMonsters, jQuery, console) {
            ...
            function Listener() {
                ...
                this.createCard = function(creature, total) {
                    creature.createCard({
                        ...
                        for (j = 0; j < data.actors.length; j++) {
                            if (data.actors[ j ].id === data.order[ i ]) {
                                ...
                                if (this.$actorCards.length) {
                                    this.createCard(actor, data.order.length);
                                }
                                break;
                            }
                        }
                ...
                this.displayImage = function(data) {
                    var img, ratio, multiplier, useWidth;
                    this.hideImage();
        ...
        //            }
                ...
                jQuery(document).ready(function() {
                    ...
                    for (i = 0; i < this.actors.length; i++) {
                        instance.createCard(this.actors[ i ], this.actors.length);
                    }

                    window.setInterval(this.handleMessage.bind(this), 1000);
                }.bind(this));
            }
