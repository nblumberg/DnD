describe("actions.js", function() {
	...
	describe("Roll", function() {
		...
		describe("new", function() {
			...
			describe("from String", function() {
				var checkField = function(s, p) {
					roll = new Roll(s);
					expect(roll[ p ]).toEqual(diceNotations[ s ][ p ]);
				};
				for (s in diceNotations) {
					it("should exist [" + s + "]", (function(s) {
						roll = new Roll(s);
						expect(roll).not.toEqual(undefined);
						expect(roll).not.toEqual(null);
					}).bind(this, s));
					it("should initialize properly [" + s + "]", (function(s) {
						roll = new Roll(s);
						expect(roll._history).toEqual([]);
						expect(roll.crits).toEqual(false);
					}).bind(this, s));
		...
		describe("roll()", function() {
			for (s in diceNotations) {
				it("should return a Number within the range allowed by the dice [" + s + "]", (function(s) {
					roll = new Roll(diceNotations[ s ]);
					result = roll.roll();
				...
				it("should add the roll to its history [" + s + "]", (function(s) {
					roll = new Roll(diceNotations[ s ]);
					expect(roll._history).toEqual([]);
					result = roll.roll();
				...
				it("each history entry should have an Array containing the rolled value of each die and the total [" + s + "]", (function(s) {
					var i;
					roll = new Roll(diceNotations[ s ]);
					result = roll.roll();
		...
		describe("max()", function() {
			for (s in diceNotations) {
				it("should return a Number that is the minimum possible value for the Roll [" + s + "]", (function(s) {
					roll = new Roll(diceNotations[ s ]);
					result = roll.max();
				...
				it("each history entry should have an Array containing the rolled value of each die, the total, and isMax: true [" + s + "]", (function(s) {
					var i;
					roll = new Roll(diceNotations[ s ]);
					result = roll.max();
					expect(roll._history[ 0 ]).not.toEqual(undefined);
		...
		describe("min()", function() {
			for (s in diceNotations) {
				it("should return a Number that is the maximum possible value for the Roll [" + s + "]", (function(s) {
					roll = new Roll(diceNotations[ s ]);
					result = roll.min();
				...
				it("each history entry should have an Array containing the rolled value of each die, the total, and isMin: true [" + s + "]", (function(s) {
					var i;
					roll = new Roll(diceNotations[ s ]);
					result = roll.min();
					expect(roll._history[ 0 ]).not.toEqual(undefined);
		...
		describe("add()", function() {
			for (s in diceNotations) {
				...
				it("should add the roll to its history [" + s + "]", (function(s, r) {
					roll = new Roll(diceNotations[ s ]);
					expect(roll._history).toEqual([]);
					roll.add(r);
					expect(roll._history.length).toEqual(1);
				...
				it("each history entry should have an Array containing the rolled value of each die, the total, and manual: true [" + s + "]", (function(s, r) {
					var i, total;
					roll = new Roll(diceNotations[ s ]);
					roll.add(r);
					expect(roll._history[ 0 ]).not.toEqual(undefined);
		...
		describe("getLastRoll()", function() {
			for (s in diceNotations) {
				it("should return the last history entry [" + s + "]", (function(s) {
					roll = new Roll(diceNotations[ s ]);
					roll.roll();
					expect(roll.getLastRoll()).toEqual(roll._history[ roll._history.length - 1 ]);
				}).bind(this, s));
				it("if there is no history, it should return an empty history entry [" + s + "]", (function(s) {
					roll = new Roll(diceNotations[ s ]);
		...
		describe("isCritical()", function() {
			for (s in diceNotations) {
				if (!diceNotations[ s ].crits || diceNotations[ s ].dieCount !== 1 || diceNotations[ s ].dieSides !== 20) {
					it("should always return false if crits: false or isn't 1d20... [" + s + "]", (function(s) {
						roll = new Roll(diceNotations[ s ]);
						roll.max();
					...
					it("should return false if there have been no rolls [" + s + "]", (function(s) {
						roll = new Roll(diceNotations[ s ]);
						expect(roll.isCritical()).toEqual(false);
					}).bind(this, s));
					it("should return false if not a die value of 20 [" + s + "]", (function(s) {
						roll = new Roll(diceNotations[ s ]);
					...
					it("should return true if a die value of 20 [" + s + "]", (function(s) {
						roll = new Roll(diceNotations[ s ]);
						roll.max();
		...
		describe("isFumble()", function() {
			for (s in diceNotations) {
				if (!diceNotations[ s ].crits || diceNotations[ s ].dieCount !== 1 || diceNotations[ s ].dieSides !== 20) {
					...
					it("should return false if not a die value of 1 [" + s + "]", (function(s) {
						roll = new Roll(diceNotations[ s ]);
					...
					it("should return true if a die value of 1 [" + s + "]", (function(s) {
						roll = new Roll(diceNotations[ s ]);
						roll.min();
		...
		describe("breakdown()", function() {
			for (s in diceNotations) {
				it("should return " + diceNotations[ s ].breakdown, (function(s) {
					roll = new Roll(diceNotations[ s ]);
					roll.max();
		...
		describe("anchor()", function() {
			for (s in diceNotations) {
				it("should return an anchor with breakdown() as the title and last roll total as innerHTML" + diceNotations[ s ].breakdown, (function(s) {
					var result, anchor;
					roll = new Roll(diceNotations[ s ]);
					result = roll.max();
					anchor = roll.anchor();
		...
		describe("raw()", function() {
			describe("should return an Object that when passed to new Roll(Object) creates an identical Roll instance", function() {
				...
				for (s in diceNotations) {
					...
					for (p in roll) {
						it(p + " [" + s + "]", (function(r1, r2, p) {
							expect(r2[ p ]).toEqual(r1[ p ]);
						}).bind(this, roll, roll2, p));
					}
				}
			});
		});
	...
	describe("Damage", function() {
		...
		describe("new", function() {
			...
			describe("from String", function() {
				var w, checkField = function(s, w, p) {
		...
		xdescribe("rollItem()", function() {
			...
			for (s in damageNotations) {
				for (w in diceNotations) {
					it("should return a Number within the range allowed by the dice [" + s + ", " + w + "]", (function(s, w) {
						var item = new Roll(w);
						damage = new Damage(damageNotations[ s ], creature);
