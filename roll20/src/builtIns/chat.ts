...
interface GeneralChatMessage extends BaseChatMessage {
  type: "general";
}

interface EmoteChatMessage extends BaseChatMessage {
  type: "emote";
}

interface DescriptionChatMessage extends BaseChatMessage {
  type: "desc";
}
 ...
 * });
 */
interface ApiChatMessage extends BaseChatMessage {
  type: "api";
...
interface WhisperChatMessage extends BaseChatMessage {
  type: "whisper";
...
interface RollResultMessage extends BaseChatMessage {
...
export type ChatMessage = ApiChatMessage | WhisperChatMessage | RollResultMessage | GeneralChatMessage | EmoteChatMessage | DescriptionChatMessage;
...
type _ChatMessage = ChatMessage; // for easier debugging
declare global {
   ...
   * @param {null | (ops: ChatMessage[]) => void} [callback] If provided, receives the resulting message objects instead of posting to chat. is an optional third parameter consisting of a callback function which will be passed the results of the sendChat() call instead of sending the commands to the game. Using sendChat() in this way is asynchronous. The results of the sendChat() command will be an ARRAY of operations, and each individual object will be just like an object you receive during a chat:message event (see above). f you want to adjust these options but do not want to use a callback parameter (third parameter--see above), you can simply pass null in it's place.
