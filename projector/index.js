(() => {
  ...
  function setEntries() {
    try {
      localStorage.setItem(
        storageKey,
        JSON.stringify(entries.sort(sortByName))
      );
    } catch (e) {}
  }
  ...
  function loadUrl() {
    ...
    entrySelect.selectedIndex = entries.indexOf(entry);
    document.querySelector(`section.left img[src="${url}"]`).scrollIntoView();
  }

  function findEntry(target) {
    const parent = target.classList.contains("entry")
      ? target
  ...
  function loadSelect(option) {
    ...
    urlInput.value = url;
    loadUrl();
    const entry = Array.from(document.getElementsByClassName("entry")).find(
  ...
  function moveEntry(target, direction) {
    ...
    parent.insertBefore(
      entryElement,
      parent.children[elementIndex + direction]
    );
  ...
  const projectedWindow = window.open(
    "projectorWindow.html",
    "projected",
    "popup,left=0,top=0,location=no,toolbar=no,menubar=no,scrollbars=no,resizable=yes"
  );
