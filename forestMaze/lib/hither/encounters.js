...
async function handleTides(_locations, _encounters, location, _fromPageLoad) {
    const hitherLocation = location;
...
const mudTrap = (priorDepth = 0) => {
    ...
    return {
        ...
        failure: makeSavingThrow(10 + depth, mudTrap.bind(null, depth), () => ({
...
export const encounters = [
    ...
    new HitherEncounter({
        ...
        failure: makeSavingThrow(10, mudTrap, () => ({
    ...
    new StreamVisionEncounter('A headless, child-sized scarecrow with metal lobster claws for hands tries on some new heads, including an upside-down wooden bucket and a withered head of cabbage. It decides on a large gourd.', 'https://www.dndbeyond.com/avatars/thumbnails/20472/380/400/402/637677437801975332.png'),
    new StreamVisionEncounter('A bullywug bedecked in the trappings of a monarch constantly adjusts his ill-fitting crown of lily blossoms while leafing through a large tome spread across his lap.', 'https://media.dndbeyond.com/compendium-images/twbtw/JtUXxjur9QWtb7E3/02-007.king-gullop.png'),
    new StreamVisionEncounter('A hag with toad-like features relaxes in a pool of water while miniature versions of her ladle the water and pour it over her head and shoulders.', 'https://www.dndbeyond.com/avatars/thumbnails/20472/350/400/471/637677436598601233.png'),
    new StreamVisionEncounter('A tall, thin figure wearing a pointy black hat and a hooded black cloak climbs a rickety staircase leading up to a large, ramshackle house built on stilts. As the figure approaches the house, their shadow seems to detach from their body, move across the walls of the house, and crawl through an open window.', 'https://media.dndbeyond.com/compendium-images/twbtw/JtUXxjur9QWtb7E3/02-009.charm.png'),
    new StreamVisionEncounter('A satyr whistles to himself as he reclines in a metal cage that dangles off the end of a boom over a lake.', 'https://cdna.artstation.com/p/assets/images/images/034/651/762/large/matthieu-martin-satyre-carre-final-web-crop.jpg?1612865327'),
    new StreamVisionEncounter('A short, mean-looking old woman wearing a crimson cap, a leather apron, and iron boots uses a cleaver to chop meat in a drab kitchen.', 'https://static.wikia.nocookie.net/the-hollow-on-netflix/images/0/0f/Red_Cap.png'),
    new StreamVisionEncounter('Two merrow swim past each other in murky water.', 'https://b2358178.smushcdn.com/2358178/wp-content/uploads/2022/06/Kiai-Sunken-Isles-Merfolk.jpg?lossy=1&strip=1&webp=1'),
    new StreamVisionEncounter('Rows upon rows of severed bullywug heads, all impaled on spikes, chatter at each other.', 
